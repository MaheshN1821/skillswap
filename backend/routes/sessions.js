...
router.post("/:bookingId/start", auth, async (req, res) => {
  try {
    ...
    if (booking.teacher.toString() !== req.userId && booking.learner.toString() !== req.userId) {
    ...
    // Generate meeting link (in real app, integrate with Zoom/Google Meet API)
...
router.post("/:bookingId/end", auth, async (req, res) => {
  try {
    ...
    if (booking.teacher.toString() !== req.userId) {
      return res.status(403).json({ message: "Only teacher can end the session" })
    }
...
router.get("/:bookingId", auth, async (req, res) => {
  try {
    ...
    if (booking.teacher._id.toString() !== req.userId && booking.learner._id.toString() !== req.userId) {
      return res.status(403).json({ message: "Not authorized" })
    }
